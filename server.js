...
app.all('/api/backup', (req, res) => {
  try {
    ...
    // Save primary backup file in the backup directory
    const mainBackupFile = path.join(BACKUP_DIR, 'tarot-codex-backup.json');
    fs.writeFileSync(mainBackupFile, jsonContent, 'utf8');

    // Save timestamped backup file in the backup directory
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const timestampedFilename = `tarot-codex-backup-${timestamp}.json`;
    const timestampedFile = path.join(BACKUP_DIR, timestampedFilename);
    fs.writeFileSync(timestampedFile, jsonContent, 'utf8');

    res.json({
      success: true,
      message: 'Backup saved directly to project backup folder.',
      filename: 'tarot-codex-backup.json',
      timestampedFilename,
      backupPath: mainBackupFile
    });
  } catch (err) {
    console.error('Backup creation failed:', err);
    res.status(500).json({ error: 'Failed to save backup file to backup folder.' });
  }
});
